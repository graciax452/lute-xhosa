...
Sources: J.C. Oosthuysen, *The Grammar of isiXhosa* (SUN PRESS, 2016), extracted directly from the PDF (see `DESIGN.md` for how — it's a scanned-image book with no text layer), for the grammar itself; [isixhosa.click](https://isixhosa.click) ([database repo](https://github.com/IsiXhosa-click/database), a UCT/SADiLaR-backed project, **licensed CC BY-SA 4.0**); and [Kaikki.org's isiXhosa Wiktionary extract](https://kaikki.org/dictionary/Xhosa/) (3,371 words, Wiktionary content is **CC BY-SA**) for the bulk of `NOUN_ROOT_LEXICON`/`VERB_ROOT_LEXICON` (1,740 noun roots, 1,079 verb roots) — see `DESIGN.md` and `DATASETS.md` for the extraction methods and the collisions each surfaced. **The Kaikki-Xhosa round in particular is larger and less thoroughly validated than everything before it** — see `DESIGN.md`'s note on scale before treating this lexicon as fully checked. Per CC BY-SA 4.0, any redistribution of this project's data must likewise credit isixhosa.click/Wiktionary and remain share-alike.
